...
int main(int argc, char *argv[])
{
  ...
  temp_file = open("/tmp/temp", O_CREAT | O_WRONLY, 0640);
  if (temp_file == -1)
  {
    print_error(module_name, "/tmp/temp", strerror(errno));
    return 1;
  }
  ...
  int read_dir_to_list(const char *curr_dir_path, struct SLList *source_list)
  {
    ...
    while ((temp_dirent = readdir(curr_dir)) != NULL)
    {
      ...
      if (temp_dirent->d_type == DT_REG)
      {
        ...
        write(temp_file, full_path, strlen(full_path));
        write(temp_file, "\n", 1);
  ...
  int substr_in_file_count(const char *file_path, const char *string2find, int *total_bytes_read)
  {
    ...
    *total_bytes_read = mem_size;
    ...
    while ((temp = memmem(temp, mem_size + mapped_mem - temp, string2find, strlen(string2find))) != NULL)
    {
      result++;
